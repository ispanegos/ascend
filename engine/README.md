...
|---|---|
| `config/` | Versioned calibration (`v0_1_0.toml`) + validation. `calibration_status = "provisional"` |
...
| `aggregation/attributes.py` | Subdomain scores, renormalized attribute aggregation |
| `confidence/confidence.py` | Coverage, recency, repeatability, quality |
| `aggregation/overall.py` | Overall (spec §8) |
| `progression/` | Peak rules, conservative updates with caps, decay |
...
.venv/bin/python -m fixtures.report                           # synthetic athletes A–G
```
